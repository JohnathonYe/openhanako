 ...
 * 3. 调 LLM 生成日记
 * 4. 存为 desk/diary/<agentName>/YYYY-MM-DD 星期X <agentName>.md（重名自动加序号）
 */
...
/** 日记单次请求输入预算（字符级近似；避免摘要+memory 撑爆上下文）。非「会话压缩」，仅截断并标注。 */
const DIARY_MAX_SUMMARY_CHARS = 72_000;
const DIARY_MAX_MEMORY_CHARS = 20_000;
const DIARY_MAX_SYSTEM_CHARS = 28_000;
const DIARY_MAX_ACTIVITIES_CHARS = 12_000;

function truncateMiddle(s, max, isZh) {
  if (!s || s.length <= max) return s;
  const head = Math.floor(max * 0.5);
  const tail = Math.floor(max * 0.42);
  const omitted = s.length - head - tail;
  const msg = isZh
    ? `\n\n---\n…（中间省略 ${omitted} 字）\n---\n\n`
    : `\n\n---\n... (${omitted} chars omitted in the middle)\n---\n\n`;
  return s.slice(0, head) + msg + s.slice(s.length - tail);
}

function truncateHead(s, max, isZh) {
  if (!s || s.length <= max) return s;
  const note = isZh
    ? `\n\n…（仅保留前 ${max} 字，其余省略）`
    : `\n\n...(only first ${max} chars kept)`;
  return s.slice(0, max) + note;
}

/** 解析日记存储目录：优先已存在的「日记/」，否则用「diary/」 */
function sanitizePathSegment(name, fallback = "unknown-agent") {
...
/** 解析日记存储目录：优先已存在的「日记/」，否则用「diary/」；如提供 agentName，则再加一层 agent 目录 */
export function resolveDiaryDir(cwd, agentName) {
  const zhDir = path.join(cwd, "日记");
  const baseDir = fs.existsSync(zhDir) ? zhDir : path.join(cwd, "diary");
  return agentName ? path.join(baseDir, sanitizePathSegment(agentName)) : baseDir;
}

function getZhWeekday(logicalDate) {
  const m = /^(\d{4})-(\d{2})-(\d{2})$/.exec(logicalDate);
  if (!m) return "星期未知";
  const [, y, mo, d] = m;
  const date = new Date(Number(y), Number(mo) - 1, Number(d), 12, 0, 0, 0);
  return ["星期天", "星期一", "星期二", "星期三", "星期四", "星期五", "星期六"][date.getDay()] || "星期未知";
}

function resolveUniqueDiaryFilePath(diaryDir, logicalDate, agentName) {
  const safeAgentName = sanitizePathSegment(agentName);
  const weekday = getZhWeekday(logicalDate);
  const baseName = `${logicalDate} ${weekday} ${safeAgentName}`;
  let fileName = `${baseName}.md`;
  let filePath = path.join(diaryDir, fileName);
  let index = 2;
  while (fs.existsSync(filePath)) {
    fileName = `${baseName} ${index}.md`;
    filePath = path.join(diaryDir, fileName);
    index += 1;
  }
  return filePath;
}
 ...
 * @param {{ logicalDate: string, rangeStart: Date, rangeEnd: Date }} [opts.logicalDay] - 覆盖默认 logical 日（手动/日界自动）
 * @param {(phase: 'writing'|'saving') => void} [opts.onProgress] - 进度回调
...
export async function writeDiary(opts) {
  const {
    ...
    cwd, activityStore,
    logicalDay: logicalDayOverride,
    onProgress,
  } = opts;

  // 1. 计算逻辑日，拉摘要
  const { logicalDate, rangeStart, rangeEnd } = logicalDayOverride || getLogicalDay();
  ...
  // 3. 构建 LLM prompt（输入过长时截断，避免超出模型上下文；不做二次 LLM「压缩」）
  const isZh = getLocale().startsWith("zh");
  const systemPrompt = truncateHead(agentPersonality || "", DIARY_MAX_SYSTEM_CHARS, isZh);
  const summaryForModel = truncateMiddle(summaryText, DIARY_MAX_SUMMARY_CHARS, isZh);
  ...
  // 活动记录（巡检 + 定时任务）
  let activitiesText = collectActivities(activityStore, rangeStart, rangeEnd);
  if (activitiesText) {
    activitiesText = truncateMiddle(activitiesText, DIARY_MAX_ACTIVITIES_CHARS, isZh);
    userPrompt.push("", "---", "",
  ...
  if (memory?.trim()) {
    const memTrim = truncateHead(memory.trim(), DIARY_MAX_MEMORY_CHARS, isZh);
    userPrompt.push("", "---", "",
  ...
  // 6. 存文件（同日重名自动追加序号）
  onProgress?.("saving");
  const diaryDir = resolveDiaryDir(cwd, agentName);
  fs.mkdirSync(diaryDir, { recursive: true });
  const filePath = resolveUniqueDiaryFilePath(diaryDir, logicalDate, agentName);
