...
export class BridgeManager {
  ...
  async _handleMessage(platform, msg) {
    ...
    if (!pending) {
      pending = { lines: [], attachments: [], platform, chatId, senderName, avatarUrl, userId, isGroup, isOwner };
    ...
    try {
      ...
      if (reply && entry?.adapter) {
        ...
        this._pushMessage({
          ...
          sender: this.engine.resolveBridgeSessionAgent(sessionKey)?.agentName || this.engine.agentName, text: cleaned,
  ...
  async _flushPending(sessionKey) {
    ...
    const { platform, chatId, senderName, avatarUrl, userId, isGroup, isOwner } = pending;
    this._pending.delete(sessionKey);
    ...
    try {
      ...
      if (isOwner && adapter) {
        await this._maybeSendBridgeHandoffFollowup(platform, chatId, sessionKey, meta, adapter);
      }
  ...
  /** handoff_service 切换后：首轮（旧助手收尾）发到平台后再由新助手发一条接棒 */
  async _maybeSendBridgeHandoffFollowup(platform, chatId, sessionKey, meta, adapter) {
    try {
      const text2 = await this.engine.runBridgeHandoffFollowupMessage(sessionKey, meta);
      if (!text2?.trim()) return;
      const cleaned = this._cleanReplyForPlatform(text2);
      if (!cleaned.trim()) return;
      const { text: textOnly, mediaUrls } = splitMediaFromOutput(cleaned);
      if (textOnly.trim()) await adapter.sendReply(chatId, textOnly);
      for (const url of mediaUrls) {
        try { await this._sendMediaItem(adapter, chatId, url); }
        catch (err) { debugLog()?.warn("bridge", `handoff follow-up media failed: ${err.message} (${url.slice(0, 60)})`); }
      }
      debugLog()?.log("bridge", `→ ${platform} handoff follow-up (${cleaned.length} chars)`);
      this._pushMessage({
        platform, direction: "out", sessionKey,
        sender: this.engine.resolveBridgeSessionAgent(sessionKey)?.agentName || this.engine.agentName,
        text: cleaned,
        isGroup: false, ts: Date.now(),
      });
    } catch (err) {
      debugLog()?.warn("bridge", `handoff follow-up failed: ${err.message}`);
    }
  }
  ...
  /**
   * 向指定平台上已绑定的本人（owner）发送一条 IM。
   * 未配置 owner、平台未启用/未连接、或 user 与设置不一致时不发送。
   *
   * @param {string} platform - telegram | feishu | qq
   * @param {string} [userId] - 若省略则从 preferences.bridge.owner[platform] 读取；若传入则须与配置一致（防误发）
   * @param {string} text - 正文（会经 _cleanReplyForPlatform 清理）
   * @returns {Promise<{ ok: true, sent: true, platform: string, chatId: string, sessionKey: string } | { ok: false, reason?: string, error?: string }>}
   */
  async sendToOwner(platform, userId, text) {
    if (!ADAPTER_REGISTRY[platform]) {
      return { ok: false, reason: "unknown_platform" };
    }

    const prefs = this.engine.getPreferences();
    const ownerConfigured = prefs.bridge?.owner?.[platform];
    if (!ownerConfigured) {
      return { ok: false, reason: "owner_not_configured" };
    }

    const ownerStr = String(ownerConfigured).trim();
    const uid = String(userId ?? "").trim();
    if (uid && uid !== ownerStr) {
      return { ok: false, reason: "user_mismatch" };
    }

    const bridgeCfg = prefs.bridge?.[platform];
    if (!bridgeCfg?.enabled) {
      return { ok: false, reason: "platform_disabled" };
    }

    const entry = this._platforms.get(platform);
    if (!entry || entry.status !== "connected" || !entry.adapter) {
    ...
    const spec = ADAPTER_REGISTRY[platform];
    const chatId = entry.adapter.resolveOwnerChatId?.(ownerConfigured) || ownerConfigured;

    try {
      await entry.adapter.sendReply(chatId, cleaned);
      debugLog()?.log("bridge", `→ ${platform} sendToOwner (${cleaned.length} chars)`);

      const sessionKey = spec?.ownerSessionKey?.(ownerConfigured) || `${platform}_dm_${ownerConfigured}`;
      this._pushMessage({
        platform, direction: "out", sessionKey,
        sender: this.engine.resolveBridgeSessionAgent(sessionKey)?.agentName || this.engine.agentName, text: cleaned,
        isGroup: false, ts: Date.now(),
      ...
      return { ok: true, sent: true, platform, chatId, sessionKey };
    } catch (err) {
      const msg = err?.message || String(err);
      console.error(`[bridge] sendToOwner failed (${platform}): ${msg}`);
      debugLog()?.error("bridge", `sendToOwner failed (${platform}): ${msg}`);
