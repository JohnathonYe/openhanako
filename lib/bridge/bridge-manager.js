...
export class BridgeManager {
  ...
  async _handleMessage(platform, msg) {
    ...
    if (!pending) {
      pending = { lines: [], platform, chatId, senderName, avatarUrl, userId, isGroup, isOwner };
      this._pending.set(sessionKey, pending);
    }
    pending.lines.push(line);
    // 更新最新的 meta（多条消息可能来自不同人，取最后一条）
    ...
    try {
      ...
      if (reply && entry?.adapter) {
        ...
        this._pushMessage({
          ...
          sender: this.engine.getBridgeAgent()?.agentName || this.engine.agentName, text: cleaned,
