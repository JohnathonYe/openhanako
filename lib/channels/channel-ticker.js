 ...
 *   opts 可含 signal、mentionedAgents（立即 triage）、isScheduledCycle+unreadSinceBookmark（定时 cycle）
...
export function createChannelTicker({
  ...
  function collectAgentChannels(agentId) {
    ...
    for (const [channelName, bookmark] of bookmarks) {
      ...
      const unreadSinceBookmark = getNewMessages(channelFile, bk, agentId);
  ...
  async function _runCycle() {
    ...
    try {
      ...
      if (agents.length === 0) {
        debugLog()?.warn("ticker", "cycle skipped: getAgentOrder() empty (no agent has channels.md)");
        return;
      }
    ...
    try {
      const result = await executeCheck(
        ...
        [],
        {
          ...
          unreadSinceBookmark: update.unreadSinceBookmark,
        },
      );
  ...
  function triggerImmediate(channelName, { mentionedAgents } = {}) {
    ...
    _triggerChain = _triggerChain.then(async () => {
      ...
      debugLog()?.log(
        "ticker",
        `triggerImmediate run #${channelName} mentionedAgents=${mentionedAgents?.length ? mentionedAgents.join(",") : "(all)"}`,
      );
  ...
  async function _doTriage(channelName, { mentionedAgents } = {}) {
    ...
    if (allAgents.length === 0) {
      debugLog()?.warn("ticker", "immediate triage: getAgentOrder() empty (no agent has channels.md)");
    }
    ...
    try {
      ...
      for (const agentId of agents) {
        ...
        if (!bookmarks.has(channelName)) {
          debugLog()?.log("ticker", `immediate skip ${agentId}: #${channelName} not in channels.md bookmarks`);
          continue;
        }
        ...
        if (recentMsgs.length === 0) {
          debugLog()?.log("ticker", `immediate skip ${agentId}: #${channelName} recentMsgs empty (bookmark/window)`);
          continue;
        }
        ...
        try {
          const result = await executeCheck(agentId, channelName, recentMsgs, [], {
            signal,
            mentionedAgents,
          ...
          debugLog()?.error("ticker", `immediate triage failed ${agentId}/#${channelName}: ${err.message}`);
        }
      }
      if (agents.length > 0 && immediateRan === 0) {
        debugLog()?.warn(
          "ticker",
          `immediate triage #${channelName}: no agent executed (all skipped: no bookmark or empty window)`,
        );
      }
    } finally {
