...
import {
  extractCompletedFromSession, extractCompletedFromReply,
  extractFailedFromReply,
  markCompletedInJian, markFailedInJian,
  normalizeJianContent, pruneCompletedFromJian,
} from "./jian-completed.js";
...
function readWorkspaceRules(wsPath) {
  if (!wsPath) return "";
  const rulesDir = path.join(wsPath, ".rules");
  try {
 ...
 */
function buildHeartbeatContext({ deskFiles, overwatch, rules, jianContent, isZh }) {
  const now = new Date();
  ...
  if (rules) {
    parts.push(isZh ? "## 必读规则（.rules/*.md）" : "## Required Rules (.rules/*.md)");
    parts.push("");
    parts.push(rules);
    parts.push("");
  }
  ...
  if (jianContent && jianContent.trim()) {
  ...
  parts.push("---");
  parts.push(isZh
    ? "请**仅根据以上提供的内容**判断是否有需要主动处理的事项。不要主动查询定时任务状态等未在上文列出的系统信息。需要通知用户时，优先用 user_notify-brother 推送到手机（level 按实际情况选），仅在该工具不可用时再用 notify 桌面弹窗。如果一切正常，不要调用任何工具。"
    : "Determine if anything needs proactive attention **based solely on the information provided above**. Do not proactively query system status such as cron jobs that is not listed above. When you need to notify the user, prefer user_notify-brother to push to their phone (choose level accordingly); only fall back to the notify desktop popup if that tool is unavailable. If everything is fine, do not call any tools.");
 ...
 */
function buildJianPrompt({ dirPath, jianContent, files, jianChanged, filesChanged, rules, isZh }) {
  const parts = isZh
    ? [
        `[目录巡检] ${dirPath}`,
        "",
  ...
  const hasJianContent = jianContent && jianContent.trim();
  if (rules && hasJianContent) {
    parts.push(isZh ? "## 必读规则（.rules/*.md）" : "## Required Rules (.rules/*.md)");
    parts.push("");
    parts.push(rules);
    parts.push("");
  ...
  parts.push("");
  parts.push(isZh
    ? "请根据笺的指令处理。任务完成或遇到阻断时，用 user_notify-brother 推送到手机通知用户。如果无需行动，不要调用任何工具。"
    : "Follow the jian instructions. When a task is completed or blocked, use user_notify-brother to push a notification to the user's phone. If no action is needed, do not call any tools.");
  parts.push("");
  parts.push(isZh
    ? "**笺中每行是一个待办任务**（`- [ ]` 待办 / `✅` 已完成 / `❌` 失败）。用 todo 工具追踪进度并在完成时 toggle 标记；巡检结束后系统自动同步到 jian.md：完成的标记 ✅，下次巡检自动移除；失败的标记 ❌，下次巡检回退为 `- [ ]` 重试。若未使用 todo，可在回复末尾输出 `<COMPLETED>该项原文</COMPLETED>`（完成）或 `<FAILED>该项原文</FAILED>`（失败），多项每行一个。"
    : "**Each line in jian is a task** (`- [ ]` pending / `✅` done / `❌` failed). Use the todo tool to track progress and toggle when done; after patrol the system syncs to jian.md: completed items get ✅ and are auto-removed next patrol; failed items get ❌ and revert to `- [ ]` for retry. If not using todo, output `<COMPLETED>item text</COMPLETED>` (done) or `<FAILED>item text</FAILED>` (failed) at the end of your reply, one per line.");
 ...
 * @returns {{ start, stop, beat, triggerNow, getManualTriggerStatus }}
 */
export function createHeartbeat({
  getDeskFiles, getWorkspacePath, registryPath,
  onBeat, onJianBeat, onJianUpdated,
  ...
  function _notifyJianUpdated(jianPath) {
    if (!onJianUpdated) return;
    try {
      const content = fs.existsSync(jianPath) ? fs.readFileSync(jianPath, "utf-8") : null;
  ...
  async function _doBeat() {
    try {
      ...
      const rules = readWorkspaceRules(wsPath) || "";
      ...
      // 读取根目录笺内容（用于 Phase 1 展示）
      let rootJianContent = null;
      if (wsPath) {
        try {
          const jp = path.join(wsPath, "jian.md");
          if (fs.existsSync(jp)) rootJianContent = fs.readFileSync(jp, "utf-8");
        } catch {}
      }
      ...
      if (deskChanged || overwatch) {
        ...
        const prompt = buildHeartbeatContext({ deskFiles, overwatch, rules, jianContent: rootJianContent, isZh });
      ...
      if (jianChanges.length > 0) {
        await _processJianChanges(jianChanges, tag, rules);
      }
   ...
   * 逐个执行有变化的笺目录
   */
  async function _processJianChanges(changes, tag, rules) {
    const registry = loadRegistry();
    ...
    for (const dir of changes) {
      ...
      // ── 自动剔除 ✅ 行，❌ 回退为 - [ ] ──
      const pruneResult = pruneCompletedFromJian(jianPath);
      if (pruneResult.pruned) {
        const parts = [];
        if (pruneResult.count > 0) parts.push(isZh ? `移除 ${pruneResult.count} 项✅` : `removed ${pruneResult.count} ✅`);
        if (pruneResult.reverted > 0) parts.push(isZh ? `回退 ${pruneResult.reverted} 项❌` : `reverted ${pruneResult.reverted} ❌`);
        const msg = `笺 [${label}] ${parts.join(", ")}`;
        console.log(`${tag}  ${msg}`);
        devlog(msg);
        _notifyJianUpdated(jianPath);
      }

      // ── 标准化纯文本行为 - [ ] 格式 ──
      if (!fs.existsSync(jianPath)) {
        devlog(isZh ? `笺 [${label}] 全部任务已完成，跳过` : `Jian [${label}] all tasks completed, skip`);
        _notifyJianUpdated(jianPath);
        continue;
      }
      let jianContent;
      try {
        jianContent = fs.readFileSync(jianPath, "utf-8");
      } catch { continue; }

      const normalized = normalizeJianContent(jianContent);
      if (normalized !== jianContent) {
        try {
          fs.writeFileSync(jianPath, normalized, "utf-8");
          jianContent = normalized;
          _notifyJianUpdated(jianPath);
        } catch {}
      }
      ...
      try {
        ...
        if (execResult && !execResult.error) {
          const completed = [
            ...extractCompletedFromSession(execResult.sessionPath),
            ...extractCompletedFromReply(execResult.replyText),
          ];
          const failed = extractFailedFromReply(execResult.replyText);
          const uniqCompleted = [...new Set(completed)];
          const uniqFailed = [...new Set(failed)];
          let synced = false;
          if (uniqCompleted.length > 0) {
            const ok = markCompletedInJian(jianPath, uniqCompleted);
            if (ok) {
              devlog(isZh ? `笺 [${label}] ✅ ${uniqCompleted.length} 项` : `Jian [${label}] ✅ ${uniqCompleted.length} items`);
              synced = true;
            }
          }
          if (uniqFailed.length > 0) {
            const ok = markFailedInJian(jianPath, uniqFailed);
            if (ok) {
              devlog(isZh ? `笺 [${label}] ❌ ${uniqFailed.length} 项` : `Jian [${label}] ❌ ${uniqFailed.length} items`);
              synced = true;
            }
          }
          if (synced) _notifyJianUpdated(jianPath);
        }
  ...
  /** 供 UI 展示立即巡检冷却与 /api/desk/heartbeat GET */
  function getManualTriggerStatus() {
    const now = Date.now();
    if (_lastTrigger === 0) {
      return { cooldownMs: COOLDOWN, remainingMs: 0, canTrigger: true };
    }
    const elapsed = now - _lastTrigger;
  ...
  return { start, stop, beat, triggerNow, getManualTriggerStatus };
}
