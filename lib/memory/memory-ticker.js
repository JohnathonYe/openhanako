...
export function createMemoryTicker(opts) {
  ...
  async function _tickCore() {
    if (!_isMemoryMasterOn()) return;
    await _tickCoreDiaryFollowup();
  }

  /**
   * 日记完成后强制跑一轮编译/组装（不受 memory.enabled 总开关阻挡；仍尊重 session 级记忆开关）
   */
  async function tickDiaryFollowup() {
    const p = _tickCoreDiaryFollowup();
    _tickInFlight = p;
    try { await p; } finally { if (_tickInFlight === p) _tickInFlight = null; }
  }

  async function _tickCoreDiaryFollowup() {
  ...
  return { start, stop, tick, tickDiaryFollowup, triggerNow, notifyTurn, notifySessionEnd, notifyPromoted, flushSession };
}
