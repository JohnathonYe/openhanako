 ...
 *   以新身份独立气泡回复转交任务
 * - Bridge 本人（社交平台 owner）私聊：同上，在本轮 bridge prompt 结束后由 BridgeSessionManager 应用队列
...
export function createServiceHandoffTool({ agentId, listAgents, getEngine, channelsDir, onChannelPost }) {
  return {
    ...
    description:
      ...
      "在已接入的社交平台（微信/Telegram 等）本人私聊中：用户说「换某某来」「让叶询接」时，用本工具切换本会话使用的助手，不要只口头答应。\n" +
    ...
    execute: async (_toolCallId, params) => {
      ...
      // ── Bridge 本人私聊：target 为 default/clear/默认 → 去掉本会话固定，恢复设置里的全局 Bridge 助手 ──
      const bridgeSkEarly = ctx?.bridgeSessionKey;
      if (bridgeSkEarly && typeof bridgeSkEarly === "string" && bridgeSkEarly.trim()) {
        const rawT = String(params.target || "").trim();
        const lowT = rawT.toLowerCase();
        if (["default", "clear", "reset", "默认", "恢复默认"].includes(lowT)) {
          const sk = bridgeSkEarly.trim();
          engine.setBridgeChatAgentForSession(sk, null);
          engine.queueBridgeImHandoffFollowup?.({ sessionKey: sk, task: "", mode: "clear" });
          return {
            content: [{
              type: "text",
              text: "已恢复为设置中的默认 Bridge 助手。请立即结束当前回复。",
            }],
            details: { mode: "bridge_im_session_clear", sessionKey: sk },
          };
        }
      }
      ...
      // ── 社交平台本人私聊（Bridge owner）：本会话固定到目标助手，清空上下文；不切换桌面主窗口 ──
      const bridgeSk = ctx?.bridgeSessionKey;
      if (bridgeSk && typeof bridgeSk === "string" && bridgeSk.trim()) {
        const sk = bridgeSk.trim();
        engine.setBridgeChatAgentForSession(sk, target.id);
        const taskNote = String(params.task || "").trim();
        engine.queueBridgeImHandoffFollowup?.({ sessionKey: sk, task: taskNote, mode: "switch" });
        return {
          content: [{
            type: "text",
            text:
              `已切换为「${target.name}」（${target.id}）。之后由该助手回复，对话上下文已继承。` +
              (taskNote ? `\n（转交说明：${taskNote}）` : "") +
              "\n请立即结束当前回复，不要继续输出正文。",
          }],
          details: { mode: "bridge_im_session", sessionKey: sk, to: target.id },
        };
      }
